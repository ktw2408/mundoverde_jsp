...
<!----------------------------------------자유게시판 주 기능--------------------------------------->
	...
	mSelectSQL += "select no,title,writer,date,type,hit ";
	mSelectSQL += "from board where type = 'D' ";
	mSelectSQL += mWhere;
	...
	while( myBoard.ResultNext() )
	{
		...
		<tr class="boardcontent">
			<td id="no"><%= no %></td>
			<td id="title"><a href="agora_view.jsp?no=<%= no %>"><%= title %></a></td>
	...
	%>
		...
		<tr>
			...
			{
				%>
				[ <a href="agoraMain.jsp?key=<%= key %>&pageno=<%= curpage - 1 %>">이전페이지</a> ] 
				<%
			}
			for(int i=1;i<=maxPage;i++)
			{
				...
				{
					%>
					&nbsp;<a href="agoraMain.jsp?&key=<%= key %>&pageno=<%= i %>"><strong><font color="red"><%= i %></font></strong></a>&nbsp;
					<%
				}else
				{
					%>
					&nbsp;<a href="agoraMain.jsp?&key=<%= key %>&pageno=<%= i %>"><%= i %></a>&nbsp;
					<%
				}
			}
			...
			{
				%>
				[ <a href="agoraMain.jsp?key=<%= key %>&pageno=<%= curpage + 1 %>">다음페이지</a> ]
				<%
			}
			%>
			</td>
		...
		<tr>
			<form id="search_form" name="search_form" method="get" action="agoraMain.jsp">
			<td id="search" colspan="5" >
				...
				<input type="button" id="write" name="write" value="글쓰기" onclick="location.href = 'agora_write.jsp?key=<%= key %>&pageno=<%= curpage %>';">
