...
<!----------------------------------------자유게시판 주 기능--------------------------------------->
	...
	<table class="freeboard" align="center" cellspacing="0">
		...
		<%
			...
			mSelectSQL += "from board where type = 'B' and league = 'I' ";
			mSelectSQL += mWhere;
		...
		%>
			...
			<tr class="boardcontent">
				<td id="no"><%= no %></td>
				<td id="title">
					<a href="inter_news_view.jsp?no=<%= no %>"><%= title %></a></td>
		...
		%>
			...
			<tr class="boardcontent">
					...
					if(curpage <= 1)
						{
							...
							{
								%>
								[ <a href="inter_news.jsp?key=<%= key %>&pageno=<%= curpage - 1 %>">
								이전페이지</a> ] 
								<%
							}
							for(int i=1;i<=maxPage;i++)
							{
								...
								{
									%>
									&nbsp;<a href="inter_news.jsp?&key=<%= key %>&pageno=<%= i %>"><strong><font color="red"><%= i %></font></strong></a>&nbsp;
									<%
								}else
								{
									%>
									&nbsp;<a href="inter_news.jsp?&key=<%= key %>&pageno=<%= i %>"><%= i %></a>&nbsp;
									<%
								}
							}
				...
				{
					%>
					[ <a href="inter_news.jsp?key=<%= key %>&pageno=<%= curpage + 1 %>">다음페이지</a> ]
					<%
				}
				%>
				</td>
			...
			<tr class="boardcontent">
			<form id="search_form" name="search_form" method="get" action="inter_news.jsp">
				<td id="search" colspan="5" >
					...
					<input type="text" id = "keyword" name="keyword" size="50" value="<%= keyword %>" placeholder="검색어를 입력하세요.">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="button" value="검색" size="5" onclick="DoSearch();">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="button" id="write" name="write" value="글쓰기" onclick="location.href = 'inter_news_write.jsp?key=<%= key %>&pageno=<%= curpage %>';">
				</td>
				</form>
			</tr>
	</table>
...
<!----------------------------------------------내용부-------------------------------------------------->	
<!--<div>
	<table border="1" align= "center" class="inter_news_list" id="_newslist">
		<tr>
			<th>
