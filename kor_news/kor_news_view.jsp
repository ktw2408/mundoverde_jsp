...
//게시물 수정
	function modifyArticle(){
			...
			window.open("kor_news_modify_ask.jsp?pageno=<%= pageno %>&no=<%= no %>","_modify","width=300,height=400");
	}
...
//게시물 삭제
	function delArticle()
	{
		...
		*/
		window.open("kor_news_delete.jsp?no=<%= no %>","_delete","width=300,height=400");
			
	}
//목록으로 이동
	function GoList()
	{
		document.location = "kor_news.jsp?key=<%= key %>&pageno=<%= pageno %>";
	}	
...
<%@ include file="kor_news_reply.jsp" %>	
</table>
