...
<!----------------------------------------자유게시판 주 기능--------------------------------------->
	...
	<table class="freeboard" align="center">
		...
		<%
			...
			mSelectSQL += "select no,title,writer,date,type,league,hit ";
			mSelectSQL += "from board where type = 'A' and league = 'K' ";
			mSelectSQL += mWhere;
		...
		<tr class="boardcontent">
			<td id="no"><%= no %></td>
			<td id="title">
			<a href="kor_view.jsp?no=<%= no %>"><%= title %></a></td>
	...
	%>
		...
		<tr class="boardcontent">
			<td id="pagenum" colspan="5" align="center">
			...
			if(curpage <= 1)
				{
					...
					{
						%>
						[ <a href="kor_free.jsp?key=<%= key %>&pageno=<%= curpage - 1 %>">이전페이지</a> ] 
						<%
					}
					for(int i=1;i<=maxPage;i++)
					{
						...
						{
							%>
							&nbsp;<a href="kor_free.jsp?&key=<%= key %>&pageno=<%= i %>"><strong><font color="red"><%= i %></font></strong></a>&nbsp;
							<%
						}else
						{
							%>
							&nbsp;<a href="kor_free.jsp?&key=<%= key %>&pageno=<%= i %>"><%= i %></a>&nbsp;
							<%
						}
					}
				...
				{
					%>
					[ <a href="kor_free.jsp?key=<%= key %>&pageno=<%= curpage + 1 %>">다음페이지</a> ]
					<%
				}
				%>
			</td>
		...
		<tr class="boardcontent">
		<form id="search_form" name="search_form" method="get" action="kor_free.jsp">
			<td id="search" colspan="5" >
				...
				<input type="button" id="write" name="write" value="글쓰기" onclick="location.href = 'kor_write.jsp?key=<%= key %>&pageno=<%= curpage %>';">
